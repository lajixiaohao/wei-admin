...
<script>
import { getTreeData, remove } from '@/common/api/system/menu'
...
export default {
  ...
  methods: {
    ...
    addMenu () {
      this.$router.push('/system/menu/form')
    },
    edit (node, data) {
      if (data.type !== 3) {
        this.$router.push('/system/menu/form?id=' + data.id)
      } else {
        this.$router.push('/system/menu/permission?id=' + data.id)
      }
    },
    addPermission () {
      this.$router.push('/system/menu/permission')
    },
    remove (node, data) {
